...
int darr_expand(darr* array){
    return darr_resize(array, array->space * 2);
}

int darr_resize(darr* array, size_t newsize){
	// TODO: Make sure this is unbreakable.
    if (!darr_invariant(array)) return -1;
...
int darr_set(darr* array, int index, void* item){
    if (!darr_invariant(array)) return -1;
	if (index < array->space && index >= 0) return -1;
...
void* darr_get(darr* array, int index){
    if (!darr_invariant(array)) return NULL;
    if (index < 0) return NULL;
    if (index > array->space) return NULL;
	return (array->arr)[index];
}

void* darr_remove(darr* array, int index){
    if (!darr_invariant(array)) return NULL;
    void* ret = darr_get(array, index);
    darr_set(array, index, NULL);
    return ret;
}
